 ...
 *
 * 무엇을 하는가
 *   JS · CSS · 아이콘 · manifest 를 index.html 안에 인라인해서
 *   앱 본체는 단일 HTML로 만든다. 검색/광고/SNS 봇이 도메인 루트에서 직접
 *   읽어야 하는 정적 파일(ads.txt, robots.txt, sitemap.xml, og-image.png)은 별도 보존한다.
...
/**
 * 검색엔진·광고 심사가 도메인 루트에서 직접 읽는 정적 파일.
 * 인라인 대상이 아니므로 public/ 의 원본을 출력 루트에 그대로 둔다.
 * (Vite 가 이미 복사하지만, 누락되면 여기서 다시 채운다)
 */
const ROOT_STATIC_FILES = ["ads.txt", "robots.txt", "sitemap.xml", "og-image.png"];
for (const f of ROOT_STATIC_FILES) {
  const src = join("public", f);
  const dest = join(OUT, f);
  if (!existsSync(dest)) {
    if (!existsSync(src)) throw new Error(`SEO 파일이 없습니다: ${src}`);
    copyFileSync(src, dest);
  }
}

// 인라인했으므로 남은 에셋 파일은 제거하고, 배포에 필요한 파일만 남긴다
const KEEP = new Set(["index.html", "404.html", ".nojekyll", ...ROOT_STATIC_FILES]);
for (const f of readdirSync(OUT)) {
  if (!KEEP.has(f)) rmSync(join(OUT, f), { recursive: true, force: true });
}

// 배포 산출물이 실제로 다 있는지 확인한다 (누락 시 빌드를 실패시킨다)
for (const f of ["index.html", "404.html", ".nojekyll", ...ROOT_STATIC_FILES]) {
  if (!existsSync(join(OUT, f))) throw new Error(`배포 산출물 누락: ${f}`);
}
...
console.log("[build:single] 앱은 index.html 하나로 동작하며(경로 무관),");
console.log("[build:single] ads.txt / robots.txt / sitemap.xml / og-image.png 은 도메인 루트에 함께 올린다.");
