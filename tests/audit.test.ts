...
describe("24. 데이터 부재·손상 시 오류 처리", () => {
  ...
  it("Result 화면이 결과 부재·매칭 실패를 모두 처리한다", () => {
    ...
    // 문구는 locale 리소스로 이동했고, 화면은 두 경우를 구분해 안내한다
    expect(result).toContain("t.result.notFoundTitle");
    expect(result).toContain("t.result.mismatchTitle");
    expect(result).toContain("matchError");
    expect(result).toContain("try {");
    const koUi = JSON.parse(src("src/locales/ko/ui.json"));
    expect(koUi.result.notFoundTitle).toBe("결과를 찾을 수 없어요");
  });
